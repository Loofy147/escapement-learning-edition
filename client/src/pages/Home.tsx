...
type Progress = { viewed: string[]; completed: string[]; attempts: Record<string, number>; mastered: string[]; current: string; positions: Record<string, { scrollY: number; sectionId: string }>; syncMeta?: { revision: number; updatedAt: number; deviceId: string } };
...
const blankProgress: Progress = { viewed: [], completed: [], attempts: {}, mastered: [], current: "ch-01", positions: {}, syncMeta: { revision: 0, updatedAt: 0, deviceId } };
...
export default function Home() {
  ...
  function updateProgress(updater: (current: Progress) => Progress) { setProgress((current) => ({ ...updater(current), syncMeta: { revision: (current.syncMeta?.revision || 0) + 1, updatedAt: Date.now(), deviceId } })); }
  function resolveConflict(choice: "local" | "remote" | "merge") { if (!remoteSnapshot) return; const resolved = resolveProgressConflict(progress, remoteSnapshot, choice); setProgress({ ...resolved, syncMeta: { revision: Math.max(progress.syncMeta?.revision || 0, remoteSnapshot.syncMeta?.revision || 0) + 1, updatedAt: Date.now(), deviceId } }); setSyncConflict("none"); }
  ...
  function grade(ex: Exercise) {
    ...
    updateProgress((current) => ({ ...current, attempts: { ...current.attempts, [ex.id]: attempts }, mastered: masteryAfterAttempt(current.mastered, ex.id, evaluated.correct) }));
    setFeedback(evaluated.state);
  }
  function resetExercise() { if (activeExercise) setActivityAnswer(activeExercise, null); setFeedback(null); setShowHint(false); }
  function sequenceValue(ex: Exercise) { return Array.isArray(answers[ex.id]) ? answers[ex.id] as number[] : []; }
  function toggleSequence(ex: Exercise, index: number) { const current = sequenceValue(ex); setActivityAnswer(ex, current.includes(index) ? current.filter((value) => value !== index) : [...current, index]); setActiveExercise(ex); setFeedback(null); }
  ...
  function setClassification(ex: Exercise, index: number, category: number) { const next = [...(Array.isArray(answers[ex.id]) ? answers[ex.id] as number[] : [])]; next[index] = category; setActivityAnswer(ex, next); setActiveExercise(ex); setFeedback(null); }
