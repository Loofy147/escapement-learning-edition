...
|---|---|---|---|---|
| BQ-001 | Ch. 5, escapement taxonomy | Recoil/deadbeat/detached/direct-impulse dimensions are partially conflated. `Detached lever` should not be presented as simply an example of a deadbeat escapement. | `REWRITE` | Use orthogonal dimensions: recoil/deadbeat locking; detached/non-detached; sliding/direct impulse. |
...
| BQ-005 | Bibliography | `[9] Derek Pratt / horological technical literature...` is not a complete bibliographic record. | `RESEARCH` | Replace with an exact work or remove citations that depend on it. |
...
| ID | Location | Finding | Action | Evidence / rationale |
|---|---|---|---|---|
| BQ-007 | Ch. 4, free-sprung balances | Free-sprung design is described too close to a guarantee of lower isochronal error. | `QUALIFY` | It changes curb-pin interaction; total isochronism still depends on spring geometry, oscillator, escapement and amplitude behavior. |
...
| BQ-013 | Ch. 5, detent | `Very high efficiency` is presented as broad fact without explicit comparative scope. | `QUALIFY` | Tie comparison to specific designs and operating conditions. |
...
| BQ-018 | Ch. 10, silicon | Silicon density/stiffness/amagnitism is used to imply improved amplitude/precision generally. | `REWRITE` | Present these as design-relevant properties whose measured benefit depends on architecture. |
| BQ-019 | Ch. 10, magnetism | Magnetized hairspring symptom is written close to a diagnosis. | `QUALIFY` | Call it a diagnostic clue, not proof; verify by test/demagnetization and observation. |
...
| BQ-022 | Ch. 16–18 | Bench procedures sometimes read as universal despite movement/caliber dependence. | `QUALIFY` | Defer movement-specific geometry, oils, tolerances and procedures to manufacturer data. |

## P1 — Standards and current-industry correctness

| ID | Location | Finding | Action | Evidence / rationale |
|---|---|---|---|---|
| BQ-023 | Ch. 12, ISO 3159 | Bibliography omits edition/status detail. | `REWRITE` | Identify ISO 3159:2009, Edition 2; distinguish current publication status from review status. |
| BQ-024 | Ch. 12, COSC | Traditional COSC scope must remain explicitly movement-level. | `KEEP + CLARIFY` | Current COSC materials distinguish traditional movement certification from finished-watch Excellence certification. |
| BQ-025 | Ch. 14/23, COSC Excellence | Current details were previously left partially as review-needed. | `UPDATE` | Use current COSC primary material and only currently published characteristics. |
| BQ-026 | Ch. 13, METAS | Current text underuses primary-source evidence. | `UPDATE` | Replace industry-described wording where supported by current METAS documentation; retain date/scope qualifications. |
| BQ-027 | Ch. 13, Rolex | 2026 Superlative Chronometer changes are missing. | `UPDATE` | Add magnetism, reliability and sustainability as additional criteria; preserve finished-watch -2/+2 s/day claim with scope. |
| BQ-028 | Ch. 13, JLC | 1000 Hours description is too tentative. | `UPDATE` | Use current manufacturer-stated description and label it proprietary. |
...
| ID | Location | Finding | Action | Evidence / rationale |
|---|---|---|---|---|
| BQ-031 | Ch. 2, H4 narrative | Jamaica trial is compressed into a simple success story. | `RESTRUCTURE` | Preserve Jamaica 1761, subsequent dispute, Barbados 1763–64, later Board consideration and Royal Observatory testing. |
| BQ-032 | Ch. 2, H4 outcome | Wording can imply one successful voyage permanently settled the longitude question. | `QUALIFY` | Say H4 demonstrated feasibility under relevant trials; do not imply that one trial ended the technical/institutional story. |
| BQ-033 | Ch. 15, Observatory Trials | Kew/Neuchâtel/scoring claims are intentionally incomplete. | `RESEARCH OR REDUCE` | Either build a source-backed historical chapter or reduce it to verified general lineage. |
| BQ-034 | Ch. 2, longitude history | The role of lunar-distance and other astronomical methods is compressed. | `QUALIFY` | Preserve as context; avoid implying the chronometer route was the only serious route. |
...
| BQ-040 | Conclusion | Future-standard ideas need clear separation from current facts. | `QUALIFY` | Label proposals as author proposals / possible future criteria. |
